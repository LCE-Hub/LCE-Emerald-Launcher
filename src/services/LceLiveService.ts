...
export class LceLiveService {
    ...
    private async request(method: string, path: string, body?: any, authed: boolean = true, retryCount: number = 0): Promise<any> {
        ...
        let res;
        try {
            res = await TauriService.httpProxyRequest(method, `${this.baseUrl}${path}`, bodyStr, headers);
        } catch (e) {
            throw new Error(`Network error when calling ${path}: ${e}`);
        }

        if (res.status === 401 && authed && this._session?.refreshToken && retryCount < MAX_REFRESH_RETRIES) {
            try {
        ...
        if (res.status >= 400) {
            const errorMsg = data.message || data.detail || data.title || data.error || `HTTP ${res.status}`;
    ...
    async startDeviceLink(): Promise<DeviceLinkStartResponse> {
        return this.request("POST", "/api/auth/device/start", {
    ...
    async pollDeviceLink(deviceCode: string): Promise<DeviceLinkPollResponse> {
        const data = await this.request("GET", `/api/auth/device/poll/${deviceCode}`, null, false);
    ...
    async sendGameInvite(recipientAccountId: string, hostIp: string, hostPort: number, hostName: string, signalingSessionId?: string): Promise<void> {
        await this.request("POST", "/api/sessions/invites", {
            recipientAccountId, hostIp, hostPort, hostName, signalingSessionId
        });
    }
